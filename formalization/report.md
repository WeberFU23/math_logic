...
### 4.2 `Rule_based_Strategy.lean`：26 个公开定理
...
### 4.3 `Rule_based_TaskProofs.lean`：42 个公开定理
...
### 4.4 `RL_based_Strategy.lean`：70 个公开定理
...
### 4.5 `RL_based_TaskProofs.lean`：46 个公开定理
...
### 4.6 `Additional_Proofs.lean`：74 个公开定理
...
| Shield 强性质 | `shield_preserves_safe_move`<br>`shield_idempotent`<br>`shield_move_output_safe`<br>`exitPushAllowed_not_walkable`<br>`shielded_total`<br>`shielded_deterministic`<br>`shielded_exists_unique` | shield 保持安全移动、具有幂等性，并且关系式输出存在且唯一 |
